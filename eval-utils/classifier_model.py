...
class ModelForSequenceClassification(nn.Module):
    ...
    def forward(self: ModelForSequenceClassification, input_data: torch.Tensor, attention_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.enc_dec:
            ...
            sigma = torch.zeros((batch_size,), device=input_data.device, dtype=torch.float32)
            output_transformer = self.transformer.get_contextualized_embeddings(input_data, attention_mask, mask_p=sigma)
        ...
        if self.take_final and not self.enc_dec:
            ...
            transformer_output = encoding[0, :] # this has dimension (1, B)
            transfomer_output = torch.transpose(transformer_output, 0, 1) # dimension (B, 1)
